...
const userController = {
  ...
  async verifyEmail(req, res) {
    try {
      ...
      return res.status(200).json({ message: "Email verification OTP sent successfully" });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: error.message });
    }
  ...
  async verifyOtp(req,res) {
    try {
      ...
      return res.status(400).json({otp: otp, userOtp: userOtp, error: "Invalid OTP Please Check Your OTP Again." });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: error.message });
    }
